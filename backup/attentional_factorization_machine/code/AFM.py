...
class AFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.attention == 1:
                self.attention_hid = tf.reshape(tf.matmul(tf.reshape(self.element_wise_product, shape=[-1, self.hidden_factor[1]]), \
                    self.weights['attention_W']), shape=[-1, self.num_interactions, self.hidden_factor[0]])
                self.attention_lin = tf.reduce_sum(tf.multiply(self.weights['attention_p'], tf.nn.relu(self.attention_hid + \
                ...
                self.attention_out = tf.reshape(tf.nn.softmax(self.attention_lin), \
                                        shape=[-1, self.num_interactions, 1], name="attention_out") # None * (M'*(M'-1)) * 1
                self.attention_out = tf.nn.dropout(self.attention_out, self.dropout_keep[0]) # dropout
