...
def parse_args():
    ...
    parser.add_argument('--hidden_factor', nargs='?', default='[64,128]', #[hidden layer nodes, embedding]
                        help='Number of hidden factors.')
    parser.add_argument('--lamda_attention', type=float, default=1e+2,
                        help='Regularizer for attention part.')
    parser.add_argument('--keep', nargs='?', default='[1.0,0.8,0.5]',
...
class AFM(BaseEstimator, TransformerMixin):
    def __init__(self, features_M, pretrain_flag, save_file, attention, hidden_factor, valid_dimension, activation_function, num_variable, layers,
        ...
        self.num_interactions = int(self.valid_dimension*(self.valid_dimension-1)/2)
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.attention == 1:
                self.attention_hid = tf.reshape(tf.matmul(tf.reshape(self.element_wise_product, shape=[-1, self.hidden_factor[1]]), # hidden_factor[1]: embedding 
                        self.weights['attention_W']),shape=[-1, self.num_interactions, self.hidden_factor[0]] # hidden_factor[0]: hidden layer nodes
                    )

                self.attention_lin = tf.reduce_sum(tf.multiply(self.weights['attention_p'], tf.nn.relu(self.attention_hid + self.weights['attention_b'])), 2, keep_dims=True) # None * (M'*(M'-1)) * 1
                self.attention_out = tf.nn.softmax(self.attention_lin, name="attention_out")
                self.attention_out = tf.nn.dropout(self.attention_out, self.dropout_keep[0]) # dropout
            elif self.attention == 2:
                self.attention_hid = tf.nn.relu(tf.matmul(self.interactions, self.weights['attention_W']) + self.weights['attention_b']) # None * h
                self.attention_lin = tf.matmul(self.attention_hid, self.weights['attention_p']) # None * (M'*(M'-1))
                self.attention_out = tf.reshape(tf.nn.softmax(self.attention_lin), shape=[-1, self.num_interactions, 1], name="attention_out") # None * (M'*(M'-1)) * 1
                self.attention_out = tf.nn.dropout(self.attention_out, self.dropout_keep[0]) # dropout
            ...
            self.AFM_FM = self.AFM_FM / self.num_interactions


            # ________ Deep Layers __________
            for i in range(0, len(self.layers)):
                self.AFM = tf.add(tf.matmul(self.AFM, self.weights['layer_%d' %i]), self.weights['bias_%d'%i]) # None * layer[i] * 1
                if self.batch_norm:
                    self.AFM = self.batch_norm_layer(self.AFM, train_phase=self.train_phase, scope_bn='bn_%d' %i) # None * layer[i] * 1
                self.AFM = self.activation_function(self.AFM)
                self.AFM = tf.nn.dropout(self.AFM, self.dropout_keep[i+1]) # dropout at each Deep layer
            # self.AFM = tf.matmul(self.AFM, self.weights['prediction'])     # None * 1
    ...
    def _initialize_weights(self):
        ...
        if self.attention == 1:
            ...
            glorot = np.sqrt(2.0 / (self.hidden_factor[0]+self.num_interactions))
            all_weights['attention_W'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.num_interactions, self.hidden_factor[0])), dtype=np.float32, name="attention_W")  # K * AK
            all_weights['attention_b'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(1, self.hidden_factor[0])), dtype=np.float32, name="attention_b")  # 1 * AK
            all_weights['attention_p'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.hidden_factor[0], self.num_interactions)), dtype=np.float32, name="attention_p") # AK


        # deep layers
        num_layer = len(self.layers)
        if num_layer > 0:
            glorot = np.sqrt(2.0 / (self.hidden_factor[1] + self.layers[0]))
            all_weights['layer_0'] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(self.hidden_factor[1], self.layers[0])), dtype=np.float32)
...
def train(args):
    ...
    if args.verbose > 0:
        print("AFM: dataset=%s, factors=%s, layers= %s, attention=%d, freeze_fm=%d, #epoch=%d, batch=%d, lr=%.4f, lambda_attention=%.1e, keep=%s, optimizer=%s, batch_norm=%d, decay=%f, activation=%s"
              %(args.dataset, args.hidden_factor, args.layers, args.attention, args.freeze_fm, args.epoch, args.batch_size, args.lr, args.lamda_attention, args.keep, args.optimizer, 
    ...
    model = AFM(data.features_M, args.pretrain, save_file, args.attention, eval(args.hidden_factor), args.valid_dimen, 
        activation_function, num_variable, eval(args.layers), args.freeze_fm, args.epoch, args.batch_size, args.lr, args.lamda_attention, eval(args.keep), args.optimizer, 
